...
require('./config')
const { default: makeWASocket, useSingleFileAuthState, DisconnectReason, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
const { smsg, getBuffer, getSizeMedia, sleep, fetchJson } = require('./lib/myfunc')
...
async function StartSocket() {
    const client = makeWASocket({
        ...
        auth: state,
        version: getVersionWaweb() || [2, 2241, 7]
    })
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./nielzie")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
// Setting
    ...
    client.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); StartSocket(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); StartSocket(); }
            ...
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); StartSocket(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); StartSocket(); }
    ...
    client.sendWelkom = async (jid, text = '', footer = '', img, but = [], options = {}) => {
        let message = await prepareWAMessageMedia({ image: img }, { upload: client.waUploadToServer })
